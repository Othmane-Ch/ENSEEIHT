...
let  rec insert ordre elt l = 
    match l with
      | []   -> [elt]
      | t::q -> if ordre t elt then t::(insert ordre elt q) else  elt::l 
...
let rec scinde l =  
  match l with
    | []        -> [],[]
    | [_]       -> l,[]
    | e1::e2::q -> let (l1,l2) = (scinde q) in  (e1::l1,e2::l2)
...
let liste_trie = tri_insertion (fun (_,_,_,x) (_,_,_,y) ->  x > y) listStatHomme

let liste_trie_f = tri_fusion (fun (_,_,_,x) (_,_,_,y) ->  x > y) listStatHomme

(*let listStat_fusion = tri_fusion (fun (_,_,_,x) (_,_,_,y) ->  x > y) listStat*)

let sort_list = List.sort (fun (_,_,_,x) (_,_,_,y) -> if  x > y then 1 else 0) listStat

let temps_sort = let t1 = Sys.time() in let l = sort_list in Sys.time() -. t1
...
(*let  scinde_terminal l =  
  let rec scinde_aux liste taille =
    match liste  with
      | []         -> [],[]
      | [_]        -> liste,[]
      | e1::e2::e3::q  -> if taille mod 0 = 0 then scinde_aux  e1::q (taille-1),scinde_aux e2::e3::q (taille-1) else scinde_aux  e1::q (taille-1),scinde_aux e2::q (taille-1)
  in scinde_aux l (List.length l)*)
